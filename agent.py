...
def search_node(state: State) -> dict:
    """itch.io 게임잼 + 공식 출처의 전시회·컨퍼런스 수집."""
...
def cap_node(state: State) -> dict:
    """주간: 전시회·컨퍼런스 전부 + 기타 신규 20건 / 채팅: cap off."""
    mode = state.get("mode", "weekly")
    if mode == "chat":
    ...
    try:
        ...
        msg = f"캡: 신규 {len(new_events)}개 중 {len(keep)}개 유지(전시회·컨퍼런스 전부 + 기타 행사 제한), {len(capped)}개 초과 제외, 기존 중복 {len(already_skipped)}개"
        logger.info(msg)
